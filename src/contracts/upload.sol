...
contract Upload {
    ...
    uint256 constant REWARD_AMOUNT = 1 * (10 ** 18); // 10 RWD tokens
    ...
    constructor(address _rwdTokenAddress) {
        rwdToken = RWD(_rwdTokenAddress);
    }
    ...
    function _rewardUser(address user , uint256 _amount) internal {
        require(rwdToken.transfer(user, _amount), "Reward transfer failed");
